...
public class RoomMB : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_scrolling == -1)
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(8, 6, 5), ScrollSpeed * Time.deltaTime);
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(-9, 6, 5), ScrollSpeed * Time.deltaTime);
